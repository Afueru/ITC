...
public class afn_debug{
	...
	public static void main (String [] args) throws Exception {
		...
		/*PrintStream saidastream = new PrintStream(saida);	//Printstream pra escrever no arquivo 'saída.txt'
		System.setOut(saidastream);*/							//Coloca a printstream como padrão
		...
		for (int automax = 0; automax < m; automax++){
			...
			System.out.println("AUTOMATO NUMERO: " + (automax + 1) + " ESTADO INICIAL: " + qo + " ACEITACAO: " + a);

			ler.nextLine();													//termina de ler a linha
			linha = ler.nextLine();											//peag a linha dos estados de aceitação
			...
			for (int i = 0; i < ntestes; i++) {								//carrega os conjuntos de teste e armazena em uma lista								
				...
				imprimeLista(testes.get(i));
			}
			System.out.println("NUMERO DE TESTES: " + testes.size());
			...
			for(int i = 0; i < testes.size(); i++) {						//percorre o conjunto de testes
				...
				System.out.println("NUMERO DE TESTES: " + (i + 1) +"/" + testes.size());
			...
			imprimeLista(resultados.get(automax));
			System.out.println("------------------------------------");
			testes.clear();
			tr.clear();
			f.clear();
		...
		System.out.println("------------------------------------");
		imprimeLista(resultados.get(0));
		System.out.println("TAMANHO: " + resultados.get(0).size());
		System.out.println("------------------------------------");
		
		//System.out.println("ALALALLAAL " + epath + "\nNUMERO: " + testes.get(0).get(0) + " Numero do entrada.txt: " + q);
	...
	public static boolean faztestes (int esatual, int iatual, ArrayList<ArrayList<Integer>> tr, ArrayList<Integer> f,ArrayList<Integer> teste) {	//método que faz os testes, recebe o estado atual, um inteiro que guarda a posição atual na lista de transições, a lista de transições, a lista de estados de aceitação e a linha de testes a ser feita
		...
		for (int g = 0; g < tr.size(); g++) { 																		//alcança o range do estado atual e checa se há alguma solução partindo dele
			if (tr.get(g).get(0) == esatual && tr.get(g).get(1) == 0) {
				...
				talquei = faztestes(tr.get(g).get(2),iatual,tr,f,teste);
				if(talquei == true) return true;																//caso haja solução, retorna true, indicando que o automato chegou no estado de aceitação
				
			}
		}
		//System.out.println("ESTADO: " + esatual);																	
																					

		if(iatual == teste.size()) {																			//condição de parada: se o elemento percorrido no array de testes for o ultimo
			//System.out.println("ESTADO FINAL DO AUTOMATO: " + esatual);
			for (int finho = 0; finho < f.size(); finho++) if(esatual == f.get(finho)) {							//percorre a lista de estados de aceitação
				//System.out.println("Ta safe pai ");
				return true;																						//se o estado atual for igual a um dos estados de aceitação, retorna true	
				}
		...
		if (tr.get(i).get(0) == esatual && tr.get(i).get(1) == teste.get(iatual)) talquei = faztestes(tr.get(i).get(2),iatual + 1,tr,f,teste);
		if (talquei == true) return true;																						//caso um teste tenha resultado em aceitação,retorna true			
	}
	return talquei;
}

	/*public static void adicionaMatriz (String linha, ArrayList<ArrayList<Integer>> a, int limite) {
		for (int i = 0; i < limite; i++){
			a.add(new ArrayList<Integer>());
			converteString(linha, a.get(i));
			imprimeLista(a.get(i));
		}
	}*/
