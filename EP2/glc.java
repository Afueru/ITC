...
public class glc {
			...
			for (int d = 1; d < cad.length; d++) {																					//"for" para iterar pelas diagonais > 1
				...
				for (int col = d; col < cad.length; col++) {																		//"for" para iterar pelos campos da diagonal atual, tri[l][col]
					...
					for (int es = l; es < col; es++) {																				//"for" para a comparação do algoritmo CYK, tri[l][(l + es)], tri[l + 1][col]
						...
						bx++;																										//avança para a linha de baixo
					}
					l++;																											//avança para a linha de baixo
				}
	...
	public static void inicializa_tri(LinkedList<LinkedList<LinkedList<String>>> tri, int tam) {							//inicializa a matriz como uma matriz quadrada
		int temp = tam;
	...
	public static void print_arr(String [] linha) {																		//função para printar array de String
		for (String ch : linha) System.out.print("[" + ch + "] ");
		System.out.println();

	}
	public static void print_mat(LinkedList<LinkedList<LinkedList<String>>>  mat) {										//função para printar uma matriz
